...
const ProgramsPage = () => {
  ...
  return (
    <>
      <Navbar />
      <section className="pt-[120px] pb-[100px] bg-cream min-h-screen" ref={ref}>
        <div className="max-w-[1200px] mx-auto px-[7%]">
          ...
          <div className={`mb-16 transition-all duration-700 delay-100 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              {ciltCourses.map((c) => (
                <div key={c.name} className="bg-background border border-border rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-2">
              {ciltShortTopics.map((t) => (
                <div key={t} className="text-sm text-foreground bg-background border border-border rounded-lg px-4 py-2.5">{t}</div>
              ))}
            </div>
          ...
          <div className={`mb-16 transition-all duration-700 delay-200 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              {accaLevels.map((l) => (
                <div key={l.name} className={`border rounded-xl p-6 ${l.highlight ? "bg-accent/5 border-accent/30" : "bg-background border-border"}`}>
          ...
          <div className={`mb-16 transition-all duration-700 delay-300 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"}`}>
            ...
            <div className="flex flex-wrap gap-3">
              {cpaLevels.map((l) => (
                <div key={l} className="bg-background border border-border rounded-xl px-6 py-4 font-semibold text-foreground">{l}</div>
              ))}
            </div>
          </div>
          ...
          {/* CTA */}
          <div className={`bg-background border border-border rounded-2xl p-10 text-center transition-all duration-700 delay-[450ms] ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"}`}>
            ...
            <div className="flex justify-center gap-3">
              ...
              <a href="https://wa.me/256773099672" className="px-7 py-3 bg-transparent border border-border text-foreground rounded-lg font-medium text-sm no-underline hover:border-accent transition-colors">WhatsApp Us</a>
            </div>
          </div>
        </div>
      </section>
